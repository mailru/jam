...
['Tmp', 'Git', 'Cache'].forEach(function (name) {

	exports['get' + name + 'Dir'] = function () {
		var root = env.home;

		if (exports.jamrc && exports.jamrc['system_dir']) {
			root = exports.jamrc['system_dir'];
		}

		root = root.replace('~', env.home);
		return path.resolve(root, '.jam', name.toLowerCase());
	};
