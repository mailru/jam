...
Repository.prototype = {
    ...
    snapshot: function(commitish, callback) {
        ...
        this.switchTo(commitish, function(err) {
            ...
            path = jamrc.getTmpDir() + '/' + [ 'git', Date.now().toString(16), Math.floor(Math.random() * (( 1 << 16) + 1)).toString(16)].join('-');
                ...
                function(next) {
                    ...
                    switch (uriParsed.protocol) {
                        case "ssh:": {
                            ...
                            uriClone = scheme + url.format(uriCloneConfig).substr(2) + delimiter + sshProject;
